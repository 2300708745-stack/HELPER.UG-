  ...
  const submit = async () => {
    ...
    try {
      if (mode === "signup") {
        ...
        if (data.user) {
          const { error: profileErr } = await supabase.from("profiles").insert([{ id: data.user.id, name: name.trim() }]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6" style={{ background: "#14213D" }}>
      <div className="w-full max-w-sm">
        ...
        <p className="text-[13px] mt-2 mb-6" style={{ color: "#8891A3" }}>Post a task or pick one up nearby.</p>

        <div className="flex gap-2 mb-5">
          <button onClick={() => setMode("signup")} className="flex-1 py-2.5 rounded-lg text-[12px] font-semibold"
            style={{ background: mode === "signup" ? "#FFB627" : "#1F2C50", color: mode === "signup" ? "#14213D" : "#8891A3" }}>
            Create account
          </button>
          <button onClick={() => setMode("login")} className="flex-1 py-2.5 rounded-lg text-[12px] font-semibold"
            style={{ background: mode === "login" ? "#FFB627" : "#1F2C50", color: mode === "login" ? "#14213D" : "#8891A3" }}>
            Log in
          </button>
        </div>
        ...
        <button onClick={submit} disabled={busy} className="w-full py-3.5 rounded-xl flex items-center justify-center gap-2"
          style={{ background: "#FFB627", color: "#14213D", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, opacity: busy ? 0.7 : 1 }}>
