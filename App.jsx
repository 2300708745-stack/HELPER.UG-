...
import {
  ...
  Smartphone, AlertCircle, Loader2, RefreshCw, Phone, MessageCircle
} from "lucide-react";
...
function TopBar({ name, verifyStatus, onSwitchUser }) {
  return (
    <div className="sticky top-0 z-30 flex items-center justify-between px-5 py-3" style={{ background: "#14213D" }}>
      ...
      <div className="flex items-center gap-2">
        {verifyStatus === "verified" && <ShieldCheck size={16} color="#2DD4A7" />}
...
function AuthScreen({ onAuthed }) {
  ...
  const submit = async () => {
    ...
    if (mode === "signup" && phone.trim().replace("+256", "").length < 9) { setError("Enter a valid phone number."); return; }
    setBusy(true);
    try {
      if (mode === "signup") {
        ...
        if (data.user) {
          const { error: profileErr } = await supabase.from("profiles").insert([{ id: data.user.id, name: name.trim(), phone: phone.trim() }]);
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden" style={{
      background: "radial-gradient(circle at 15% 0%, #2D6A4F 0%, #14213D 45%), radial-gradient(circle at 100% 100%, #E4572E 0%, transparent 40%), #14213D",
    }}>
      {/* decorative pattern layer */}
      <div className="absolute inset-0 pointer-events-none" style={{ opacity: 0.5 }}>
        <svg width="100%" height="260" viewBox="0 0 400 260" preserveAspectRatio="xMidYMid slice">
          <circle cx="40" cy="40" r="70" fill="#FFB627" opacity="0.15" />
          <circle cx="340" cy="30" r="50" fill="#2D6A4F" opacity="0.25" />
          <circle cx="370" cy="180" r="90" fill="#E4572E" opacity="0.12" />
          <circle cx="30" cy="200" r="40" fill="#FFB627" opacity="0.1" />
        </svg>
      </div>

      <div className="relative px-6 pt-14 pb-8">
        <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5 shadow-lg" style={{ background: "linear-gradient(135deg, #FFB627, #E4572E)" }}>
          <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 22, color: "#14213D" }}>Q</span>
        </div>
        <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 28, color: "#FFF" }}>Get things done in Kampala</h1>
        <p className="text-[13px] mt-2" style={{ color: "#B7C0D8" }}>Post a task or pick one up nearby.</p>
      </div>

      <div className="relative flex-1 px-6 pb-10">
        <div className="w-full max-w-sm mx-auto rounded-2xl p-5" style={{ background: "rgba(20,33,61,0.55)", backdropFilter: "blur(4px)", border: "1px solid rgba(255,255,255,0.08)" }}>
        <div className="flex gap-2 mb-5">
          <button onClick={() => setMode("signup")} className="flex-1 py-2.5 rounded-lg text-[12px] font-semibold"
            style={{ background: mode === "signup" ? "linear-gradient(135deg, #FFB627, #E4572E)" : "#1F2C50", color: mode === "signup" ? "#14213D" : "#8891A3" }}>
            Create account
          </button>
          <button onClick={() => setMode("login")} className="flex-1 py-2.5 rounded-lg text-[12px] font-semibold"
            style={{ background: mode === "login" ? "linear-gradient(135deg, #FFB627, #E4572E)" : "#1F2C50", color: mode === "login" ? "#14213D" : "#8891A3" }}>
            Log in
          </button>
        </div>
        ...
        {mode === "signup" && (
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name"
            className="w-full rounded-xl px-4 py-3.5 text-[14px] outline-none mb-3" style={{ background: "#1F2C50", color: "#FFF" }} />
        )}
        {mode === "signup" && (
        ...
        {error && (
          <div className="flex items-center gap-2 text-[12px] mb-3" style={{ color: "#FF8A65" }}>
            <AlertCircle size={13} /> {error}
          </div>
        )}

        <button onClick={submit} disabled={busy} className="w-full py-3.5 rounded-xl flex items-center justify-center gap-2"
          style={{ background: "linear-gradient(135deg, #FFB627, #FFC960)", color: "#14213D", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, opacity: busy ? 0.7 : 1 }}>
          {busy ? <Loader2 size={16} className="animate-spin" /> : mode === "signup" ? "Create account" : "Log in"}
        </button>
        </div>
      </div>
    </div>
...
function VerifyBadge({ status }) {
  if (status === "verified") {
    return (
      <span className="inline-flex items-center gap-1 text-[10px] font-bold tracking-wide px-2 py-0.5 rounded" style={{ background: "#E6F2EC", color: "#2D6A4F", fontFamily: "'IBM Plex Mono', monospace" }}>
  ...
  if (status === "pending") {
    return (
      <span className="inline-flex items-center gap-1 text-[10px] font-bold tracking-wide px-2 py-0.5 rounded" style={{ background: "#FFF3D6", color: "#B77400", fontFamily: "'IBM Plex Mono', monospace" }}>
        PENDING REVIEW
      </span>
    );
  }
  if (status === "rejected") {
    return (
      <span className="inline-flex items-center gap-1 text-[10px] font-bold tracking-wide px-2 py-0.5 rounded" style={{ background: "#FDEDE8", color: "#E4572E", fontFamily: "'IBM Plex Mono', monospace" }}>
        RESUBMIT ID
      </span>
    );
  }
...
function VerifyIDScreen({ session, status, onUploaded }) {
  ...
  const submit = async () => {
    if (!file) { setError("Choose a photo of your ID first."); return; }
    setUploading(true); setError(null);
    try {
      const path = `${session.user.id}/id-${Date.now()}.jpg`;
      const { error: uploadErr } = await supabase.storage.from("id-documents").upload(path, file);
      if (uploadErr) throw uploadErr;
      const { error: profileErr } = await supabase.from("profiles").update({ verification_status: "pending", id_document_path: path }).eq("id", session.user.id);
      if (profileErr) throw profileErr;
      onUploaded();
    } catch (e) {
  ...
  if (status === "verified") {
    return (
      <div className="mx-5 rounded-xl p-5 flex items-center gap-3" style={{ background: "#E6F2EC", border: "1px solid #2D6A4F" }}>
        <ShieldCheck size={22} color="#2D6A4F" />
        <div>
          <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 14, color: "#14213D" }}>Your ID is verified</div>
          <div className="text-[11px] mt-0.5" style={{ color: "#6B7280" }}>Other users can see your verified badge.</div>
        </div>
      </div>
    );
  }
  ...
  if (status === "pending") {
    return (
      <div className="mx-5 rounded-xl p-5 flex items-center gap-3" style={{ background: "#FFF3D6", border: "1px solid #FFB627" }}>
        <ShieldCheck size={22} color="#B77400" />
        <div>
          <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 14, color: "#14213D" }}>Your ID is under review</div>
          <div className="text-[11px] mt-0.5" style={{ color: "#6B7280" }}>This is usually reviewed within a day or two.</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-5 rounded-xl p-5" style={{ background: "#FFFFFF", border: "1px solid #ECE9DF" }}>
      <div className="flex items-center gap-2 mb-1">
        <ShieldCheck size={17} color="#14213D" />
        <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 14, color: "#14213D" }}>
          {status === "rejected" ? "Resubmit your ID" : "Verify your identity"}
        </span>
      </div>
      <p className="text-[12px] mb-3" style={{ color: "#6B7280" }}>
        Upload a clear photo of your National ID (or equivalent). This builds trust with other users and unlocks a verified badge.
      </p>

      {preview ? (
        <img src={preview} alt="ID preview" className="w-full rounded-lg mb-3" style={{ maxHeight: 160, objectFit: "cover" }} />
      ) : (
        <label className="flex flex-col items-center gap-2 py-6 rounded-lg mb-3 cursor-pointer" style={{ background: "#F7F4EC", border: "1px dashed #C9C5B8" }}>
          <Smartphone size={20} color="#6B7280" />
          <span className="text-[12px]" style={{ color: "#6B7280" }}>Tap to choose a photo</span>
          <input type="file" accept="image/*" capture="environment" onChange={pickFile} className="hidden" />
        </label>
      )}

      {error && <div className="flex items-center gap-2 text-[12px] mb-3" style={{ color: "#E4572E" }}><AlertCircle size={13} /> {error}</div>}

      <button onClick={submit} disabled={uploading} className="w-full py-3 rounded-xl flex items-center justify-center gap-2"
        style={{ background: "#FFB627", color: "#14213D", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, opacity: uploading ? 0.7 : 1 }}>
        {uploading ? <Loader2 size={16} className="animate-spin" /> : "Submit for review"}
      </button>
    </div>
  );
}
...
function HomeScreen({ name, tasks, loading, error, onRetry, goPost, goBrowse, setTab }) {
  ...
  return (
    <div>
      <div className="relative overflow-hidden px-5 pt-6 pb-8" style={{
        background: "radial-gradient(circle at 90% -10%, #2D6A4F 0%, transparent 55%), linear-gradient(160deg, #14213D 0%, #1B2C52 100%)",
      }}>
        <svg className="absolute top-0 right-0 pointer-events-none" width="180" height="180" viewBox="0 0 180 180" style={{ opacity: 0.35 }}>
          <circle cx="150" cy="20" r="60" fill="#FFB627" opacity="0.25" />
          <circle cx="170" cy="120" r="35" fill="#E4572E" opacity="0.3" />
        </svg>
        <div className="relative">
          <div className="text-[10px] font-semibold tracking-[0.2em] mb-1" style={{ color: "#FFB627", fontFamily: "'IBM Plex Mono', monospace" }}>HI, {name.toUpperCase()}</div>
          <h1 style={{ color: "#FFF", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 22 }}>What do you need done today?</h1>
        </div>
      </div>
      <div className="px-5 -mt-4 mb-5 relative">
        <button onClick={goPost} className="w-full flex items-center justify-between px-5 py-4 rounded-2xl shadow-lg" style={{ background: "linear-gradient(135deg, #FFB627, #E4572E)" }}>
      ...
      <div className="px-5 grid grid-cols-2 gap-3 mb-6">
        <div className="rounded-xl p-4" style={{ background: "linear-gradient(135deg, #FFFFFF, #FDF6E8)", border: "1px solid #ECE9DF" }}>
        ...
        <button onClick={goBrowse} className="rounded-xl p-4 text-left" style={{ background: "linear-gradient(135deg, #14213D, #2D6A4F)" }}>
            ...
            {CATEGORIES.map((c) => {
              ...
              return (
                <button key={c.id} onClick={() => setCat(c.id)} className="flex flex-col items-start gap-3 p-4 rounded-xl text-left"
                  ...
                  <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: selected ? "rgba(255,182,39,0.18)" : `linear-gradient(135deg, ${c.color}33, ${c.color}14)` }}>
                    <Icon size={17} color={selected ? "#FFB627" : c.color} />
                  </div>
...
function BrowseScreen({ name, tasks, loading, error, onRetry, onAccept, acceptingId }) {
  ...
  return (
    <div>
      <div className="relative overflow-hidden px-5 pt-6 pb-7" style={{
        background: "radial-gradient(circle at 10% -10%, #E4572E 0%, transparent 50%), linear-gradient(160deg, #14213D 0%, #1B2C52 100%)",
      }}>
        <svg className="absolute bottom-0 left-0 pointer-events-none" width="160" height="120" viewBox="0 0 160 120" style={{ opacity: 0.3 }}>
          <circle cx="10" cy="100" r="55" fill="#FFB627" opacity="0.25" />
          <circle cx="90" cy="30" r="25" fill="#2D6A4F" opacity="0.3" />
        </svg>
        <div className="relative">
          <div className="text-[10px] font-semibold tracking-[0.2em] mb-1" style={{ color: "#FFB627", fontFamily: "'IBM Plex Mono', monospace" }}>{open.length} NEARBY</div>
          <h1 style={{ color: "#FFF", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 22 }}>Tasks you could pick up</h1>
        </div>
      </div>
      <div className="pt-5">
...
function ChatScreen({ task, name, onBack }) {
  ...
  const otherPerson = task.requester_name === name ? task.helper_name : task.requester_name;

  const loadMessages = useCallback(async () => {
    setLoading(true); setError(null);
    const { data, error: err } = await supabase.from("messages").select("*").eq("task_id", task.id).order("created_at", { ascending: true });
    if (err) setError("Couldn't load messages.");
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`messages-task-${task.id}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages", filter: `task_id=eq.${task.id}` }, () => loadMessages())
      .subscribe();
  ...
  const send = async () => {
    ...
    setSending(true);
    const { error: err } = await supabase.from("messages").insert([{ task_id: task.id, sender_name: name, body: draft.trim(), created_at: Date.now() }]);
  ...
  return (
    <div className="flex flex-col" style={{ height: "100%" }}>
      <div className="sticky top-0 z-30 flex items-center gap-3 px-5 py-3" style={{ background: "#14213D" }}>
        <button onClick={onBack} className="text-[13px] font-semibold" style={{ color: "#FFB627" }}>← Back</button>
        <div>
          <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 14, color: "#FFF" }}>{otherPerson || "Chat"}</div>
          <div className="text-[10px]" style={{ color: "#8891A3" }}>{task.title}</div>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-2">
        {loading && <Spinner label="Loading messages…" />}
        {error && <ErrorBanner message={error} onRetry={loadMessages} />}
        {!loading && !error && messages.length === 0 && (
          <div className="mx-auto mt-8 rounded-xl p-6 text-center" style={{ background: "#FFFFFF", border: "1px dashed #C9C5B8" }}>
            <p className="text-[12px]" style={{ color: "#6B7280" }}>No messages yet — say hello and confirm the details.</p>
          </div>
        )}
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
              <div className="max-w-[75%] rounded-2xl px-4 py-2.5" style={{
                background: mine ? "linear-gradient(135deg, #FFB627, #FFC960)" : "#FFFFFF",
                border: mine ? "none" : "1px solid #ECE9DF",
              }}>
                <div className="text-[13px]" style={{ color: "#14213D" }}>{m.body}</div>
              </div>
            </div>
          );
        })}
      ...
      <div className="sticky bottom-0 flex items-center gap-2 px-5 py-3" style={{ background: "#F7F4EC", borderTop: "1px solid #ECE9DF" }}>
        <input value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()}
          placeholder="Type a message…" className="flex-1 rounded-full px-4 py-2.5 text-[13px] outline-none" style={{ background: "#FFFFFF", border: "1px solid #ECE9DF", color: "#14213D" }} />
        <button onClick={send} disabled={sending || !draft.trim()} className="w-10 h-10 rounded-full flex items-center justify-center"
          style={{ background: "linear-gradient(135deg, #FFB627, #E4572E)", opacity: sending || !draft.trim() ? 0.6 : 1 }}>
          {sending ? <Loader2 size={15} className="animate-spin" color="#14213D" /> : <Send size={15} color="#14213D" />}
        </button>
      </div>
    </div>
  );
...
function MyTasksScreen({ name, session, verifyStatus, onVerifyUpdated, tasks, loading, error, onRetry, onComplete, onRate, busyId, onOpenChat }) {
    ...
    <div>
      <div className="px-5 pt-5">
        <div className="rounded-2xl p-5 flex items-center gap-4" style={{ background: "#14213D" }}>
          <div className="w-14 h-14 rounded-full flex items-center justify-center" style={{ background: "linear-gradient(135deg, #FFB627, #E4572E)" }}>
            <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 19, color: "#14213D" }}>{name.slice(0, 2).toUpperCase()}</span>
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 16, color: "#FFF" }}>{name}</span>
      ...
      <div className="mt-4">
        <VerifyIDScreen session={session} status={verifyStatus} onUploaded={onVerifyUpdated} />
      </div>
      ...
      <div className="px-5 flex flex-col gap-4 pb-6">
        {mine.map((t) => {
          ...
          const canChat = t.helper_name && (t.status === "accepted" || t.status === "completed");
          const otherPhone = iAmRequester ? t.helper_phone : t.requester_phone;
          const waNumber = otherPhone ? otherPhone.replace(/[^0-9]/g, "") : null;
          const commButtons = canChat ? (
            <div className="flex items-center gap-1.5">
              {otherPhone && (
                <a href={`tel:${otherPhone}`} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "#1F2C50" }}>
                  <Phone size={13} color="#FFB627" />
                </a>
              )}
              {waNumber && (
                <a href={`https://wa.me/${waNumber}`} target="_blank" rel="noreferrer" className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "#1F2C50" }}>
                  <MessageCircle size={13} color="#2DD4A7" />
                </a>
              )}
              <button onClick={() => onOpenChat(t)} className="text-[10.5px] font-bold px-3 py-2 rounded-lg" style={{ background: "#1F2C50", color: "#FFB627", fontFamily: "'IBM Plex Mono', monospace" }}>
                CHAT
              </button>
            </div>
          ) : null;
          return (
            <TaskTicket key={t.id} task={t} viewerName={name} footer={
              (footer || commButtons) ? <div className="flex items-center gap-2 flex-wrap justify-end">{commButtons}{footer}</div> : null
            } />
          );
        })}
      </div>
...
export default function App() {
  ...
  const [verifyStatus, setVerifyStatus] = useState("not_submitted");
  ...
  const loadSessionAndProfile = useCallback(async () => {
    ...
    setSession(s);
    if (s) {
      const { data: profile } = await supabase.from("profiles").select("name, phone, verification_status").eq("id", s.user.id).single();
      setName(profile ? profile.name : s.user.email);
      setPhone(profile ? profile.phone : "");
      setVerifyStatus(profile ? profile.verification_status : "not_submitted");
    } else {
      setName(null);
    }
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange(() => { loadSessionAndProfile(); });
  ...
  const handleCreate = async (task) => {
    const { error: err } = await supabase.from("tasks").insert([{ ...task, requester_phone: phone, created_at: Date.now() }]);
    if (err) throw err;
    await loadTasks();
    setTab("mine");
  };
  ...
  const handleAccept = async (id) => {
    setAcceptingId(id);
    const { error: err } = await supabase.from("tasks").update({ status: "accepted", helper_name: name, helper_phone: phone }).eq("id", id);
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "#14213D" }}>
        <Loader2 size={22} className="animate-spin" color="#FFB627" />
      </div>
    );
  }

  if (!session || !name) return <AuthScreen onAuthed={loadSessionAndProfile} />;
    ...
    return (
      <div className="min-h-screen flex justify-center" style={{ background: "#EDEAE0" }}>
        <div className="w-full max-w-md min-h-screen flex flex-col" style={{ background: "#F7F4EC" }}>
          <ChatScreen task={chatTask} name={name} onBack={() => setChatTask(null)} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex justify-center" style={{ background: "#EDEAE0" }}>
      <div className="w-full max-w-md min-h-screen flex flex-col" style={{ background: "#F7F4EC" }}>
        <TopBar name={name} verifyStatus={verifyStatus} onSwitchUser={switchUser} />
        <div className="flex-1 overflow-y-auto">
          ...
          {tab === "mine" && <MyTasksScreen name={name} session={session} verifyStatus={verifyStatus} onVerifyUpdated={loadSessionAndProfile} tasks={tasks} loading={loading} error={error} onRetry={loadTasks} onComplete={handleComplete} onRate={handleRate} busyId={busyId} onOpenChat={setChatTask} />}
        </div>
